public class MidPoint_Symetry {
    ...
    static void MidPoint(int x1,int y1,int x2,int y2,int z){
        ...
        int i=0;
        int zone=z;
        System.out.println("D: "+d+" dNE: "+dNE+" dE: "+dE);
        while(i<=10){
            if(d<=0){
                System.out.println(X+" "+Y+" D: "+d+" E "+" D_update "+(d+dE)+" Original: "+ConvrtO_to_X(X,Y,zone)+" "+ConvrtO_to_Y(X,Y,zone));
                d=d+dE;
                X++;
            }
            else{
                System.out.println(X+" "+Y+" D: "+d+" NE "+" D_update "+(d+dNE)+" Original: "+ConvrtO_to_X(X,Y,zone)+" "+ConvrtO_to_Y(X,Y,zone));
                d=d+dNE;
                X++;
                Y++;
            }
